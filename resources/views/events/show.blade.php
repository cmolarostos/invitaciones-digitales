        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5">
            ...
            <form method="POST" action="{{ route('events.photos.store', $event) }}"
                ...
                <div class="flex items-center gap-3">
                    <label class="flex items-center gap-2 cursor-pointer text-sm border border-dashed border-gray-300 rounded px-3 py-1.5 hover:bg-gray-50 transition">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        <span id="file-label">Seleccionar fotos</span>
                        <input type="file" name="photos[]" multiple accept="image/*" class="hidden" id="photo-input">
                    </label>
                    <button type="submit" id="upload-btn"
                            class="text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded px-3 py-1.5 disabled:opacity-50 disabled:cursor-not-allowed hidden">
                        Subir
                    </button>
                </div>
                <p class="text-xs text-gray-400 mt-1">Máx. 2 MB por imagen · JPG, PNG, WEBP</p>
                @error('photos')
            ...
            (function () {
                ...
                input.addEventListener('change', function () {
                    if (this.files.length === 0) { btn.classList.add('hidden'); return; }
